    ...
    Column {
        ...
        LaunchedEffect(key1 = textValue.value, block = {
            ...
            if (textValue.value.isEmpty()) {
                errorValue.value = context.getString(R.string.email_cant_empty)
            } else if (!Patterns.EMAIL_ADDRESS.matcher(textValue.value).matches()) {
                errorValue.value = context.getString(R.string.email_invalid)
            }
        })
